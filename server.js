...
app.get("/addstd", (req,res)  =>{
    const {first_name, last_name, phone_number} = req.query;
    let qry2 =  "select * from records WHERE phone_number=?";
        ...
        if(err)
          throw err
            ...
            if(results.length > 0){
              ...
              let qry = " insert into records values(?,?,?)";  
              mysql.query(qry, [first_name, last_name, phone_number], (err, results) =>{
...
app.get("/searchstd", (req,res) =>{
    const {first_name, last_name, phone_number} = req.query
    let qry = " SELECT first_name, last_name, phone_number FROM records WHERE first_name LIKE ?";  
    mysql.query(qry, [first_name, last_name, phone_number], (err, results) =>{
        if (err)
             throw err
        else{
...
app.get("/deletestd", (req, res) => {
    ...
    let que = "delete from records where first_name=?";
...
app.get("/updatestd", (req,res) =>{
        ...
        let qry = "select * from records WHERE phone_number=?"; 
    ...
    const {first_name, last_name, phone_number} = req.query;
    let qry = "update records set first_name=?, last_name=? where phone_number=?";
    mysql.query(qry, [first_name, last_name, phone_number], (err,results)=>{
        if(err) throw err
        else{
            if(results.affectedRows > 0 ){
